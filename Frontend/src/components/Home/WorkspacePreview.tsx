        ...
        <section className="bg-[#FAF6ED] py-24 px-4 sm:px-4 lg:px-4 overflow-hidden">
            <div className="max-w-[87rem] mx-auto border rounded-4xl border-zinc-200 bg-gradient-to-r from-[#C5E2C6]/80 via-[#C5E2C6]/40 via-10% to-transparent to-70%">
                <div className="grid grid-cols-1 p-4 mx-3 lg:p-6 lg:grid-cols-12 gap-12 lg:gap-8 items-center ">
                    ...
                    >
                        {/* BADGE: font-mono */}
                        <span className="font-mono text-emerald-800 text-md font-medium tracking-wider uppercase mb-7">
                        ...
                        {/* PARAGRAPH: font-sans */}
                        <p className="font-sans text-zinc-600 leading-relaxed mb-8 text-base">
                        ...
                        <button className="flex items-center gap-3 text-zinc-900 font-medium w-fit group cursor-pointer transition-transform duration-300 hover:translate-x-1.5">
                            ...
                            {/* BUTTON TEXT: font-sans */}
                            <span className="font-sans text-base font-semibold group-hover:text-emerald-800 transition-colors duration-300">See It In Action</span>
                        </button>
                    </motion.div>
                    ...
                    >
                        ...
                        <div className="h-14 border-b border-zinc-200 bg-white px-6 flex items-center justify-between">
                            <div className="flex items-center gap-2 cursor-pointer group">
                                {/* LOGO: font-serif & font-mono combo */}
                                <span className="font-serif font-bold text-zinc-900 text-sm tracking-wide transition-colors duration-300 group-hover:text-amber-800">
                                    <span className="text-amber-700 mr-1 transition-transform duration-500 inline-block group-hover:rotate-12 group-hover:scale-110">✦</span>AKASHIX<span className="font-mono text-amber-700">CORE</span>
                                </span>
                            </div>
                            ...
                            <div className="hidden sm:flex items-center bg-zinc-50 hover:bg-zinc-100 border border-zinc-200 hover:border-zinc-300 rounded-lg px-3 py-1.5 w-60 md:w-68 text-zinc-400 gap-2 text-xs transition-all duration-300 cursor-text">
                                ...
                                {/* SEARCH: font-sans */}
                                <span className="font-sans">Search your world...</span>
                            </div>

                            <div className="flex items-center gap-3">
                                {/* NEW BUTTON: font-sans */}
                                <button className="font-sans bg-emerald-900 hover:bg-emerald-800 text-white font-medium px-3 py-1.5 rounded-lg flex items-center gap-1.5 transition-all duration-300 hover:scale-105 active:scale-95 hover:shadow-[0_0_10px_rgba(6,95,70,0.3)] text-xs shadow-2xs cursor-pointer">
                        ...
                        <div className="flex min-h-[460px]">
                            ...
                            <div className="hidden md:flex flex-col w-52 border-r border-zinc-200 bg-[#FAF9F5] p-3.5 space-y-3">
                                {/* SIDEBAR LABEL: font-mono */}
                                <div className="font-mono text-[10px] uppercase tracking-wider text-zinc-400 font-semibold px-2">WORKSPACE</div>
                                ...
                                <div className="group bg-white border border-zinc-200 hover:border-zinc-300 rounded-lg px-2.5 py-2 flex items-center justify-between text-zinc-800 font-medium shadow-2xs cursor-pointer text-xs transition-all duration-300 hover:shadow-sm">
                                    {/* WORKSPACE NAME: font-sans */}
                                    <span className="font-sans truncate group-hover:text-emerald-800 transition-colors duration-300">Chronicles of Elaria</span>
                                ...
                                <div className="space-y-0.5 pt-1">
                                    {navItems.map(({ icon: Icon, label, active }) => (
                                        ...
                                        >
                                            ...
                                            {/* NAV LINKS: font-sans */}
                                            <span className="font-sans transition-colors duration-300 group-hover:text-zinc-900">{label}</span>
                                        </div>
                                    ))}
                                </div>
                            </div>
                            ...
                            <div className="flex-1 p-5 sm:p-6 bg-white grid grid-cols-1 lg:grid-cols-[1.4fr_1fr] gap-5">
                                    ...
                                    <div className="grid grid-cols-3 gap-2.5">
                                        {quickStats.map((s) => (
                                            <div key={s.label} className="group border border-zinc-200 hover:border-emerald-200 rounded-xl p-3 bg-white shadow-2xs hover:shadow-md transition-all duration-300 hover:-translate-y-1 cursor-default">
                                                {/* STAT LABEL: font-sans + uppercase spacing */}
                                                <div className="font-sans uppercase tracking-wider text-zinc-400 text-[9px] mb-1 transition-colors duration-300 group-hover:text-emerald-700/70">{s.label}</div>
                                                {/* STAT VALUE: font-mono */}
                                                <div className="font-mono font-semibold text-zinc-900 text-lg mb-0.5 transition-transform duration-300 origin-left group-hover:scale-105 group-hover:text-emerald-900">{s.value}</div>
                                                {/* STAT DELTA: font-mono */}
                                                <div className="font-mono text-emerald-700 text-[10px] font-medium">{s.delta}</div>
                                            </div>
                                        ))}
                                    </div>

                                    <div className="border border-zinc-200 rounded-xl p-4 bg-white shadow-2xs">
                                        {/* ACTIVITY LIST TITLE: font-sans */}
                                        <div className="font-sans text-zinc-900 font-semibold text-xs mb-3">Recent Activity</div>
                                            ...
                                            {activity.map((item, i) => (
                                                <li key={i} className="group flex items-center justify-between text-xs cursor-default">
                                                    <div className="flex items-center gap-2 text-zinc-700 truncate pr-2 transition-transform duration-300 group-hover:translate-x-1">
                                                        ...
                                                        {/* ACTIVITY TEXT: font-sans */}
                                                        <span className="font-sans truncate text-zinc-800 transition-colors duration-300 group-hover:text-emerald-800">{item.text}</span>
                                                    </div>
                                                    {/* ACTIVITY TIME: font-mono */}
                                                    <span className="font-mono text-zinc-400 text-[11px] shrink-0 transition-colors duration-300 group-hover:text-zinc-500">{item.time}</span>
                                                </li>
                                            ))}
                                        </ul>
                                    </div>
                                ...
                                <div className="group flex flex-col rounded-xl overflow-hidden border border-zinc-200 hover:border-zinc-400 relative bg-zinc-900 min-h-[300px] justify-between p-4 bg-[url('https://wallpapercave.com/wp/wp12519636.jpg')] bg-cover bg-center shadow-inner transition-all duration-500 hover:shadow-lg">
                                    ...
                                    <div className="relative z-10">
                                        {/* MAP TAG: font-mono + uppercase tracking for technical feel */}
                                        <span className="font-mono uppercase tracking-wider text-white font-medium text-[10px] bg-black/40 backdrop-blur-md px-2.5 py-1 rounded-md border border-white/15 transition-all duration-300 group-hover:border-white/30 group-hover:bg-black/60">
                                            Elaria Continent
                                        </span>
                                    </div>

                                    <div className="relative z-10 w-full">
                                        {/* MAP BUTTON: font-sans */}
                                        <button className="font-sans w-full bg-white/15 hover:bg-white/30 backdrop-blur-md text-white font-medium py-2 rounded-lg border border-white/25 hover:border-white/40 transition-all duration-300 text-center text-xs shadow-sm cursor-pointer hover:scale-[1.02] active:scale-95 hover:shadow-[0_0_15px_rgba(255,255,255,0.2)]">
                                            Open World Map
                                        </button>
                                    </div>
                                </div>
