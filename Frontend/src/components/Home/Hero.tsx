        ...
        >
            ...
            <div className="max-w-7xl mx-auto px-4 sm:px-2 py-10 relative z-10 w-full mt-10">
                <div className="grid lg:grid-cols-2 gap-10 items-center">
                    ...
                    >
                        {/* BADGE: font-mono for a technical system tag look */}
                        <div className="font-mono group inline-flex items-center gap-2 border border-amber-500/30 bg-amber-500/10 hover:bg-amber-500/20 rounded-full px-3 py-1 mb-6 text-xs text-amber-400 font-semibold tracking-widest uppercase transition-colors duration-300 cursor-default">
                        ...
                        {/* SUBHEADING: font-sans for clean readability */}
                        <p className="font-sans text-zinc-300 text-lg leading-relaxed max-w-md mb-10">
                            AkashixCore is the all-in-one worldbuilding and narrative design
                            platform for writers, by writers.
                        </p>

                        <div className="flex flex-wrap gap-4 mb-16">
                            {/* PRIMARY BUTTON: font-sans */}
                            <Link
                                to="/register"
                                className="font-sans group bg-[#d9a05b] hover:bg-[#e8b577] text-black font-bold px-6 py-3.5 rounded-lg flex items-center gap-2 transition-all duration-300 shadow-[0_0_20px_rgba(217,160,91,0.3)] hover:shadow-[0_0_30px_rgba(217,160,91,0.6)] hover:scale-105 active:scale-95"
                            >
                                Start Building Free <ArrowRight className="w-4 h-4 transition-transform duration-300 group-hover:translate-x-1.5" />
                            </Link>

                            {/* SECONDARY BUTTON: font-sans */}
                            <button className="font-sans group bg-zinc-900/50 backdrop-blur-sm border border-white/20 hover:bg-zinc-800 hover:border-white/40 text-white font-semibold px-6 py-3.5 rounded-lg flex items-center gap-2 transition-all duration-300 hover:scale-105 active:scale-95 hover:shadow-[0_0_20px_-5px_rgba(255,255,255,0.2)]">
                        ...
                        <div className="flex flex-wrap gap-8 lg:gap-12">
                            {stats.map(({ icon: Icon, value, label }) => (
                                <div key={label} className="group flex items-center gap-3 cursor-default">
                                    ...
                                    <div>
                                        {/* STAT VALUE: font-mono for raw data/numbers */}
                                        <div className="font-mono text-white text-xl font-bold leading-none mb-1 transition-transform duration-300 group-hover:translate-x-0.5">{value}</div>
                                        {/* STAT LABEL: font-sans, uppercase, and slightly spaced out */}
                                        <div className="font-sans uppercase tracking-wider text-zinc-400 text-xs font-semibold transition-colors duration-300 group-hover:text-zinc-300">{label}</div>
                                    </div>
                                </div>
                            ))}
                        </div>
                    ...
                    >
                        <div className="group bg-[#081B21]/70 backdrop-blur-sm border border-white/10 hover:border-[#d9a05b]/30 rounded-2xl p-10 max-w-sm w-full shadow-2xl transition-all duration-700 hover:-translate-y-2 hover:shadow-[0_20px_40px_-10px_rgba(217,160,91,0.15)]">
                            {/* CARD HEADING: font-serif to match the main H1 */}
                            <h3 className="font-serif text-white text-xl font-bold leading-snug mb-8 transition-colors duration-500 group-hover:text-zinc-100">
                                Everything you need to bring your universe to life.
                            </h3>
                            <ul className="space-y-5">
                                {checklist.map((item) => (
                                    /* CHECKLIST ITEMS: font-sans */
                                    <li key={item} className="font-sans group/item flex items-center gap-3 text-sm text-zinc-300 hover:text-white font-medium transition-colors duration-300 cursor-default">
