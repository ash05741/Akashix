...
export const Characters = () => {
    ...
    return (
        <div className="space-y-8 relative font-sans text-zinc-900">
            {/* Header */}
            <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
                ...
                <button
                    ...
                    className="flex items-center gap-2 rounded-xl bg-[#0F2C24] hover:bg-[#153b30] px-5 py-2.5 text-xs font-bold uppercase tracking-wider text-white transition-colors shadow-sm shrink-0"
                >
            ...
            {/* Empty State / Grid */}
            {characters.length === 0 ? (
                <div className="flex flex-col items-center justify-center rounded-2xl border border-dashed border-zinc-300 bg-white/50 py-20 text-center shadow-sm">
                ...
                <div className="grid grid-cols-1 gap-6 sm:grid-cols-2 lg:grid-cols-3">
                    {characters.map((char) => (
                        <div
                            ...
                            className="rounded-2xl border border-zinc-200 bg-white p-6 hover:border-amber-400/60 hover:-translate-y-1 transition-all duration-300 shadow-sm group cursor-pointer hover:shadow-md flex flex-col h-full"
                        >
                            {/* Card Header */}
                            <div className="flex items-start justify-between mb-6">
                                <div className="flex items-center gap-4">
                                    <div className="flex h-12 w-12 items-center justify-center rounded-xl bg-[#081B21] shadow-inner shrink-0">
                                ...
                                <button
                                    ...
                                    className="text-zinc-400 hover:text-red-600 hover:bg-red-50 p-1.5 rounded-lg transition-colors opacity-0 group-hover:opacity-100 shrink-0"
                                ...
                                {char.relatedLore && char.relatedLore.length > 0 ? (
                                    <div className="flex flex-wrap gap-2">
                                        {char.relatedLore.slice(0, 2).map(lore => (
                                            <span key={lore.id} className="inline-flex items-center gap-1.5 px-2.5 py-1 bg-amber-50 border border-amber-200 rounded-lg text-[9px] uppercase font-bold tracking-widest text-amber-800 shadow-sm">
                                                <LinkIcon className="w-3 h-3 text-amber-500" />
                                                {lore.title}
                                            </span>
                                        ))}
                                        {char.relatedLore.length > 2 && (
                                            <span className="px-2.5 py-1 bg-zinc-50 border border-zinc-200 rounded-lg text-[9px] font-bold text-zinc-500 uppercase tracking-widest shadow-sm">
            ...
            {selectedChar && (
                <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/40 backdrop-blur-sm" onClick={() => setSelectedChar(null)}>
                    ...
                    >
                        <div className="flex justify-between items-center p-5 border-b border-zinc-100 bg-zinc-50 shrink-0">
                            <div className="flex items-center gap-4">
                                <div className="flex h-12 w-12 items-center justify-center rounded-xl bg-[#081B21] shadow-inner">
                                    <User className="h-6 w-6 text-[#d9a05b]" strokeWidth={1.5} />
                                </div>
                                <div>
                                    <h2 className="font-serif text-xl font-bold text-zinc-900 mb-1 tracking-tight">{selectedChar.name}</h2>
                                    <span className="text-[9px] font-bold text-zinc-500 uppercase tracking-widest border border-zinc-200 bg-white px-2 py-0.5 rounded-md shadow-sm">Entity Profile</span>
                                </div>
                            </div>
                        ...
                        <div className="p-6 overflow-y-auto custom-scrollbar space-y-6 bg-[#FAF6ED]/30">
                                ...
                                {selectedChar.relatedLore && selectedChar.relatedLore.length > 0 ? (
                                    <div className="flex flex-wrap gap-2">
                                        {selectedChar.relatedLore.map(lore => (
                                            <span key={lore.id} className="inline-flex items-center gap-1.5 px-3 py-1.5 bg-amber-50 border border-amber-200 rounded-lg text-[10px] font-bold tracking-widest uppercase text-amber-800 shadow-sm">
                                    ...
                                    <p className="text-xs text-zinc-500 italic bg-white p-3 rounded-xl border border-zinc-200 shadow-sm">
                            ...
                            <div>
                                ...
                                <div className="grid grid-cols-3 gap-3">
                                    <div className="bg-white p-3 rounded-xl border border-zinc-200 shadow-sm flex flex-col items-center justify-center">
            ...
            {isModalOpen && (
                <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 backdrop-blur-sm p-4">
                    <div className="w-full max-w-lg rounded-2xl border border-zinc-200 bg-white flex flex-col max-h-[85vh] shadow-2xl">
                        <div className="flex items-center justify-between p-5 border-b border-zinc-100 bg-zinc-50 shrink-0">
                            ...
                            <form id="create-char-form" onSubmit={handleCreate} className="space-y-5">
                                <div className="space-y-4">
                                    <div>
                                        ...
                                        <input
                                            ...
                                            className="block w-full rounded-lg border border-zinc-200 bg-white px-3 py-2.5 text-zinc-900 placeholder-zinc-400 focus:border-[#d9a05b] focus:outline-none focus:ring-1 focus:ring-[#d9a05b] shadow-sm transition-all text-sm"
                                        />
                                    </div>
                                    <div>
                                        ...
                                        <textarea
                                            ...
                                            className="block w-full rounded-lg border border-zinc-200 bg-white px-3 py-2.5 text-zinc-900 placeholder-zinc-400 focus:border-[#d9a05b] focus:outline-none focus:ring-1 focus:ring-[#d9a05b] resize-none custom-scrollbar shadow-sm transition-all text-sm"
                                        />
                                    </div>
                                </div>
                                ...
                                <div>
                                    ...
                                    {availableLore.length === 0 ? (
                                        <div className="p-3 bg-white border border-zinc-200 rounded-lg text-xs text-zinc-400 italic shadow-sm">
                                            No world lore found. Create lore first.
                                        </div>
                                    ) : (
                                        <div className="flex flex-wrap gap-2 max-h-32 overflow-y-auto p-2 border border-zinc-200 rounded-lg bg-white shadow-inner custom-scrollbar">
                                            {availableLore.map((lore) => (
                                                <button
                                                    key={lore.id}
                                                    type="button"
                                                    onClick={() => toggleLoreSelection(lore.id)}
                                                    className={`px-2.5 py-1 rounded-md text-[10px] uppercase tracking-wider font-bold transition-all border cursor-pointer ${selectedLoreIds.includes(lore.id)
                        ...
                        <div className="p-5 border-t border-zinc-100 bg-white shrink-0 flex justify-end gap-2">
                            <button type="button" onClick={() => setIsModalOpen(false)} className="rounded-lg px-4 py-2 text-xs font-bold uppercase tracking-wider text-zinc-500 hover:text-zinc-900 hover:bg-zinc-50 transition-colors cursor-pointer">Cancel</button>
                            <button form="create-char-form" type="submit" disabled={creating} className="flex items-center justify-center min-w-[100px] rounded-lg bg-[#0F2C24] hover:bg-[#153b30] px-5 py-2 text-xs font-bold uppercase tracking-wider text-white disabled:opacity-50 transition-colors shadow-sm cursor-pointer">
                                {creating ? <Loader2 className="w-3.5 h-3.5 animate-spin text-[#d9a05b]" /> : 'Deploy'}
                            </button>
                        </div>
                    </div>
                </div>
            ...
            {charToDelete && (
                <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 backdrop-blur-sm p-4">
                    <div className="w-full max-w-sm rounded-2xl border border-red-100 bg-white p-6 shadow-2xl overflow-hidden relative">
                        <div className="flex items-center gap-3 mb-4">
                            <div className="flex items-center justify-center w-10 h-10 rounded-xl bg-red-50 border border-red-100 shrink-0">
                                <AlertTriangle className="w-5 h-5 text-red-600" />
                            </div>
                            <div>
                                <h3 className="font-serif text-lg font-bold text-zinc-900 leading-tight">Delete Character?</h3>
                        ...
                        <div className="flex justify-end gap-2">
                            <button
                                ...
                                disabled={deleting}
                                className="rounded-lg px-4 py-2 text-xs font-bold uppercase tracking-wider text-zinc-500 hover:text-zinc-900 hover:bg-zinc-50 transition-colors cursor-pointer"
                            >
                                Cancel
                            </button>
                            <button
                                ...
                                disabled={deleting}
                                className="flex items-center justify-center min-w-[90px] rounded-lg bg-red-600 hover:bg-red-700 px-4 py-2 text-xs font-bold uppercase tracking-wider text-white disabled:opacity-50 transition-colors shadow-sm cursor-pointer"
                            >
